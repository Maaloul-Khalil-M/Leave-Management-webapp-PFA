 ...
 * Idempotent: skips if calendars collection is non-empty.
 */
...
public class _0003__SeedCalendar {
    ...
    public void rollback(MongoDatabase db) {
        db.getCollection("calendar_days").deleteMany(new Document());
        db.getCollection("calendars").deleteMany(new Document());
    }
    ...
    private void seedCalendars(MongoDatabase db) {
        ...
        col.insertMany(List.of(
                new Document("_id", CALENDAR_TN_ID)
                        .append("code", "TN-2025-2026")
                        .append("name", "Tunisia 2025-2026")
                        .append("country", "TN")
                        .append("year", 2026)
                        .append("createdAt", now)
                        .append("updatedAt", now),

                new Document("_id", CALENDAR_FR_ID)
                        .append("code", "FR-2025-2026")
                        .append("name", "France 2025-2026")
                        .append("country", "FR")
                        .append("year", 2026)
                        .append("createdAt", now)
                        .append("updatedAt", now)
    ...
    private void seedCalendarDays(MongoDatabase db) {
        ...
        addHoliday(days, CALENDAR_TN_ID, "2025-09-04", "Mouled (Prophet's Birthday)", now);
        ...
        // TN special exceptions (illustrative)
        addDay(days, CALENDAR_TN_ID, "2026-08-14", "SPECIAL_NON_WORKING_DAY",
                "Company bridge day", now);
        addDay(days, CALENDAR_TN_ID, "2026-09-12", "SPECIAL_WORKING_DAY",
    ...
    private static void addHoliday(List<Document> target, String calendarId,
        ...
        addDay(target, calendarId, isoDate, "PUBLIC_HOLIDAY", label, now);
    }

    private static void addDay(List<Document> target, String calendarId,
                               String isoDate, String dayType, String label, Instant now) {
        target.add(new Document()
