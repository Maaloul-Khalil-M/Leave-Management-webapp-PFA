...
public class _0004__SeedDemoWorkforce {
    ...
    private void seedEmployees(MongoDatabase db, Instant now) {
        ...
        // ---- Manager: Salma ----
        col.insertOne(employee(
                EMP_SALMA_ID, "MGR-001", "ACTIVE",
                profile("Salma", "Trabelsi", "F",
                        LocalDate.of(1988, 4, 12), "salma@acme.tn", "+21620000001",
                        LocalDate.of(2018, 3, 1)),
                assignment(DEPT_ENGINEERING_ID, "Engineering",
                        POS_ENGINEERING_MANAGER_ID, "Engineering Manager",
                        LocalDate.of(2022, 1, 1), "TN"),
                null, now));

        // ---- Ahmed (Backend, reports to Salma) ----
        col.insertOne(employee(
                EMP_AHMED_ID, "EMP-001", "ACTIVE",
                profile("Ahmed", "Ben Salah", "M",
                        LocalDate.of(1994, 3, 11), "ahmed@acme.tn", "+21620000002",
        ...
        col.insertOne(employee(
                EMP_YOSRA_ID, "EMP-002", "ACTIVE",
                profile("Yosra", "Khelifi", "F",
                        LocalDate.of(1996, 8, 20), "yosra@acme.tn", "+21620000003",
        ...
        // ---- Leila (HR, standalone) ----
        col.insertOne(employee(
                EMP_LEILA_ID, "HR-001", "ACTIVE",
                profile("Leila", "Mansouri", "F",
                        LocalDate.of(1990, 1, 15), "leila@acme.tn", "+21620000004",
                        LocalDate.of(2019, 6, 1)),
                assignment(DEPT_HR_ID, "Human Resources",
                        POS_HR_SPECIALIST_ID, "HR Specialist",
                        LocalDate.of(2019, 6, 1), "TN"),
                null, now));

        // ---- Karim (TN site, similar role to Ahmed) ----
        col.insertOne(employee(
                EMP_KARIM_ID, "EMP-TN-001", "ACTIVE",
                profile("Karim", "Ben Ammar", "M",
                        LocalDate.of(1992, 5, 14), "karim@acme.tn", "+21620000010",
        ...
        col.insertOne(employee(
                EMP_CAMILLE_ID, "EMP-FR-001", "ACTIVE",
                profile("Camille", "Dupont", "F",
                        LocalDate.of(1990, 11, 3), "camille@acme.fr", "+33140000020",
    ...
    private void seedUsers(MongoDatabase db, Instant now) {
        ...
        col.insertOne(user(USER_SALMA_ID, "salma@acme.tn", EMP_SALMA_ID, now));
        col.insertOne(user(USER_AHMED_ID, "ahmed@acme.tn", EMP_AHMED_ID, now));
        col.insertOne(user(USER_YOSRA_ID, "yosra@acme.tn", EMP_YOSRA_ID, now));
    ...
    private static Document assignment(String deptId, String deptLabel,
                                       String posId, String posLabel,
                                       LocalDate start, String countryCode) {
        return new Document()
    ...
    private static Document user(String id, String email, String employeeId, Instant now) {
        return new Document("_id", id)
                .append("email", email)
                .append("employeeId", employeeId)
                .append("accountStatus", "ACTIVE")
                .append("identity", new Document()
                        .append("provider", "KEYCLOAK")
                        .append("subject", id + "-keycloak-sub"))
                .append("createdAt", now)
                .append("updatedAt", now);
    }
}
